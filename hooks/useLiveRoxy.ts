...
export const useLiveRoxy = () => {
  ...
  const checkAndRequestPermissions = async (): Promise<boolean> => {
      ...
      // Check if we have successfully granted it before
      const hasMicAccess = localStorage.getItem('ROXY_MIC_GRANTED');

      if (!hasMicAccess) {
          // If not documented, ASK USER FIRST via our Custom UI
          // This ensures the user clicks "Allow" on our UI, providing a fresh user gesture
          // for the subsequent navigator.mediaDevices call.
          const confirmed = await requestPermissionUI('audio');
          if (!confirmed) {
              setError("Microphone access is required for ROXY.");
              return false;
          }
      }
      ...
      try {
          ...
          // If successful, stop tracks immediately (we'll reopen when connecting)
          stream.getTracks().forEach(t => t.stop());
          // Mark as granted
          localStorage.setItem('ROXY_MIC_GRANTED', 'true');
      } catch (e) {
          console.error("Mic Permission Failed", e);
          // If it fails here, it's likely a system-level block or previous denial
          setError("Microphone permission denied by system.");
          return false;
      }

      // 2. Camera (Optional)
      // We ask upfront to ensure seamless switching later, but it's not "required" for boot
      const hasCamAccess = localStorage.getItem('ROXY_CAM_GRANTED');
      if (!hasCamAccess) {
           ...
           if (confirmed) {
               try {
                   ...
                   // We don't block app start for camera failure
               }
           }
      }

      // 3. Overlay (Simulation)
      const hasOverlayAccess = localStorage.getItem('ROXY_OVERLAY_GRANTED');
      if (!hasOverlayAccess) {
